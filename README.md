...
当前第 33 课：已实现知识库 CRUD、登录权限、文档上传与解析、分块、流式聊天、向量实验、文档索引与知识库多文档 RAG 问答，支持检索评估、混合检索与可选 GLM 重排。MySQL 保存元数据与索引状态，MinIO 保存原文件，Qdrant 保存向量；回答使用 GLM 并展示跨文档引用原文。
...
- [第 33 课：候选重排，让 GLM 再判断片段与问题的关系](docs/lesson-33.md)

知识库问答新增默认关闭的 GLM 重排，最多六段候选中选择并排序三段，并显示同次请求的重排前后对照。开启后通常额外调用一次现有 GLM，不需新模型密钥或数据库迁移；需重启新版后端。原分数保留，排序效果需结合评估和原文判断。
