...
账号创建入口为 /register，登录入口为 /login。升级后先运行初始化脚本补充 JWT_SECRET，再启动后端；已有凭据保留。登录状态仅在内存保存，刷新需要重新登录。第十五课已保护知识库全部增删改查接口；未登录访问业务页面会跳转到登录页。所有登录用户暂时共享知识库，下一课进入角色权限。后续按照手册推进路由、Spring Boot、MySQL、登录与权限、文档管理、LLM 问答与 RAG、测试及部署。
