...
当前第 35 课：已实现知识库 CRUD、登录权限、文档上传与解析、分块、流式聊天、向量实验、文档索引与知识库多文档 RAG 问答，支持检索评估、混合检索、可选 BGE 重排、耗时统计和问题向量缓存。MySQL 保存元数据与索引状态，MinIO 保存原文件，Qdrant 保存向量；回答使用 GLM 并展示跨文档引用原文。
