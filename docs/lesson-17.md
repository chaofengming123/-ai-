...
如果脚本提示数据库操作失败，先确认 Docker Desktop 已启动，再运行：

~~~sh
docker compose --env-file docker/.env -f docker/compose.yml ps
~~~

mysql 服务应显示为 healthy。脚本现在会同时打印数据库返回的具体原因，便于区分“Docker 未启动”“数据库尚未迁移”等问题。

如果 IDEA 启动后端时提示 8080 已被使用，说明已经有另一份后端在运行。不要同时启动两份；可以继续使用现有进程，或者先在原来的终端按 Ctrl+C 停止它，再从 IDEA 启动。本课排查时已停止课程后台进程并释放 8080。
