...
文件格式和上传大小沿用上一课：UTF-8 TXT / Markdown，单个文件不超过 1 MB。本课重点是文件存在哪里、怎样找到它，而非文档解析。
...
bucket 指定桶，object 指定 key，stream 指定内容及已知长度。If-None-Match 表示目标已存在时不覆盖。本课内容最多 1 MB，因此使用内存中的字节数组即可；以后支持大文件时再设计流式处理。
