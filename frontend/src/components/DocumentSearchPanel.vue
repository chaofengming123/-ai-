...
import { searchDocument, answerDocument } from '../api/documents.js'
...
const { result, busy, error, run, reset } = createVectorStorage({
  search: async ({ text, mode }, signal) => ({
    ...(await (mode === 'answer' ? answerDocument : searchDocument)(props.documentId, text, signal)), kind: mode,
  }),
}, () => auth.sessionVersion)
...
<template>
  <section class="chunk-panel" aria-label="文档检索">
    <h3>检索与文档问答 · 第 29 课</h3>
    <p>“查找片段”仅将问题发送到向量模型。“基于文档回答”还会将问题及最多三个检索片段发送到 GLM，并展示回答和引用原文。</p>
    <p v-if="!available">需要先有与当前模型匹配的成功索引；建立或重建后刷新索引状态。</p>
    <p v-if="!canSearch">检索需要同时拥有文档查看和 AI 对话权限。</p>
    <form class="chat-form" @submit.prevent="canSearch && available && run('search', { text: query, mode: 'search' })">
      ...
      <button class="primary-button" :disabled="busy || !available || !canSearch || !query.trim()">{{ busy ? '正在处理……' : '查找最相近的 3 个片段' }}</button>
      <button type="button" class="primary-button" :disabled="busy || !available || !canSearch || !query.trim()" @click="run('search', { text: query, mode: 'answer' })">{{ busy ? '正在处理，请等待……' : '基于文档回答' }}</button>
    </form>
    ...
    <section v-if="result" aria-label="文档检索结果">
      ...
      <template v-if="result.kind === 'answer'">
        <h4>{{ result.insufficient ? '当前资料不足' : '基于片段的回答' }}</h4>
        <pre class="document-text-preview">{{ result.answer }}</pre>
        <p>回答模型：{{ result.model }}。引用编号已校验，但请对照原文核实答案。</p>
        <article v-for="source in result.sources" :key="source.sourceId" class="chunk-item">
          <h4>引用 {{ source.sourceId }} · 块 {{ source.chunkIndex + 1 }}</h4>
