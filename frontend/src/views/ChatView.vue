...
const { messages, draft, isBusy, error, notice, send, reset, stop } = createStreamConversation(
  (messages, signal, delta) => sendChatStream(messages, signal, delta, auth), () => auth.sessionVersion)
...
<template>
  <section class="page" aria-labelledby="chat-title">
    ...
    <p class="demo-note">流式模型对话：回复会逐步显示。发送的问题和最近几轮完整对话会交给模型服务，当前不读取知识库文档。离开页面或刷新后清空对话。</p>
    ...
    <div v-else>
      ...
      <div ref="transcript" class="chat-transcript" role="log" aria-label="对话记录" aria-live="polite">
        ...
        <p v-if="isBusy" role="status">正在接收模型回复……</p>
      </div>
