...
test('interceptors attach only local protected tokens and ignore old 401 responses', async () => {
  ...
  try {
    ...
    assert.equal((await client.get('/chat/config')).data, 'Bearer first')
    assert.equal((await client.post('/chat', {})).data, 'Bearer first')
    assert.equal((await client.post('/embeddings/compare', {})).data, 'Bearer first')
